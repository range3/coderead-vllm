...
| `docs/src/components/multimodal/mm-processing.md` | フロントエンドMM処理。チャットテンプレート適用、プレースホルダー展開、トークン列構造、**MMハッシュ[DEEP]**（hash_kwargs 3層構造、serialize_item型別シリアライズ、画像3パス[EXIF UUID/MediaWithBytes/ピクセル]、iter_item_to_bytes再帰展開、_hash_mm_itemsのmm_uuids分岐、identifier vs mm_hash使い分け、プレフィックスキャッシュextra_keys連携、Gemma3具体例）、ProcessorCache 4種(processor_only/lru/shm/none)、P0-P1キャッシュ整合性、MultiModalFeatureSpec構築 | [MEDIUM→DEEP(§3)] | [VERIFIED] | 2026-02-17 |
