...
| 2026-02-18 | 2h+ | mm_cache内部実装調査。MultiModalConfig 3設定項目（デフォルト値確認）、5クラス実装詳細（LRUCache/SingleWriterShmRingBuffer）、キャッシュタイプ選択ロジック（IPC対応可否判定）、CPU並列処理（OMP_NUM_THREADS/inter-request並列化機構なし） | `docs/src/investigations/mm-cache-internals.md` |
